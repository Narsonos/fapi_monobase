...
from app.application.intefraces import AuthStrategy, TokenMixin, PasswordMixin, LoginLogoutMixin
...
class StatefulOAuthStrategy(AuthStrategy, TokenMixin, PasswordMixin, LoginLogoutMixin):

    def __init__(self, session_repo: SessionRepository, user_repo: UserRepository, password_hasher: domsvc.PasswordHasher):
    ...
    async def login(self, credentials: dict) -> TokenResponse:
        ...
        if not self.hasher.hash(password, user.password_hash):
