import pytest, random, pytest_asyncio as pytestaio
...
@pytestaio.fixture
async def valid_user() -> dmod.User:
    return await dmod.User.create(
        username='test',
        password='12341234',
        role='user',
        hasher=AsyncHasherAdapter(FakeHasher())
    )
...
@pytest.mark.asyncio
async def test_invalid_username_raises_error(username, hasher, expected_error):
    if expected_error:
        with pytest.raises(domexc.UserValueError, match=expected_error):
            await dmod.User.create(
                username = username,
                password = '123123123',
...
@pytest.mark.asyncio
async def test_password_length(valid_user: dmod.User, hasher: AsyncHasherAdapter, password, expected_error):
    if expected_error: 
        with pytest.raises(domexc.UserValueError, match=expected_error):
            await valid_user.force_change_password(new=password, hasher=hasher)
    else:
        await valid_user.force_change_password(new=password, hasher=hasher)
        assert await hasher.verify(password, valid_user.password_hash)
...
@pytest.mark.models
@pytest.mark.asyncio
async def test_hash_password(hasher: AsyncHasherAdapter):
    with pytest.raises(domexc.UserValueError):
        await dmod.User._hash_password('1234567',hasher)

    password = '12345678'
    hash = await dmod.User._hash_password(password, hasher)
    assert await hasher.verify(hashed=hash, password=password)


@pytest.mark.models
@pytest.mark.asyncio
async def test_user_creation(hasher: AsyncHasherAdapter):
    data = dict(
        username = 'test',
        role=dmod.Role.USER,
    )
    password = '11112222'
    user = dmod.User(**data, password_hash=await hasher.hash(password), status=dmod.Status.ACTIVE)
    created_user = await dmod.User.create(**data, password=password, hasher=hasher)
...
@pytest.mark.asyncio
async def test_password_change(valid_user: dmod.User, hasher: AsyncHasherAdapter, old, new, expected_error):
    if expected_error == 'Old password invalid':
        with pytest.raises(domexc.UserValueError, match=expected_error):
            await valid_user.change_password('not_matching_old', new, hasher)
    elif expected_error == 'password must not match the old one':
        with pytest.raises(domexc.UserValueError,match=expected_error):
            await valid_user.change_password(old, old, hasher)
    elif expected_error is None:
        await valid_user.change_password(old, new, hasher)
        assert await hasher.verify(password=new, hashed=valid_user.password_hash)
